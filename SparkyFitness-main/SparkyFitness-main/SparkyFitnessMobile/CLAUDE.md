...
- `App.tsx` — Root providers: `QueryClientProvider` → `GestureHandlerRootView` → `KeyboardProvider` → `BottomSheetModalProvider` → `NavigationContainer` → `SafeAreaProvider` → `Toast`. Every screen is wrapped with `withErrorBoundary(...)` (from `ScreenErrorBoundary`) before being registered with the navigator, so a crashing screen falls back to a graceful in-place error UI instead of crashing the whole app.
- **Root Stack Navigator** (`@react-navigation/stack`): `Onboarding` (shown when no server config exists) + `Tabs` screen + food entry flow (`FoodSearch`, `FoodForm`, `FoodEntryAdd`, `FoodEntryView`, `FoodScan`) + exercise/workout flow (`ExerciseSearch` (modal), `PresetSearch`, `WorkoutAdd`, `ActivityAdd`, `WorkoutDetail`, `ActivityDetail`) + settings subscreens (`CalorieSettings`, `FoodSettings`) + `Logs` + `Sync` screen. Dynamic initial route: `Onboarding` if no `ServerConfig` exists, otherwise `Tabs`.
...
- **services/** — Organized into subdirectories:
  ...
  - Top-level: `healthConnectService.ts`/`.ios.ts` (platform orchestration), `backgroundSyncService`, `storage`, `LogService`, `themeService`, `calculations`, `seedHealthData.ts`/`.ios.ts`, `workoutDraftService` (AsyncStorage-backed draft persistence for in-progress workout/activity forms), `healthDataDisplay` (centralized health metric display formatting — `fetchHealthDisplayData` returns per-metric formatted strings), `diagnosticReportService` (general app diagnostic report builder — collects app/device info, sync status, sanitized logs, health metrics, preferences, cache states for sharing), `healthDiagnosticService` (Android-only Health Connect record diagnostic report — reads raw records with privacy-aware rounding, exports `shareHealthDiagnosticReport`), `notifications` (expo-notifications + expo-haptics wrapper for the workout rest timer — `initNotifications`, `ensureNotificationPermission`, `scheduleRestNotification`, `cancelScheduledNotification`, `fireRestCompleteHaptic`; sets up the `workout-timer` Android channel and uses a silent foreground handler so only the haptic/sound fire when the app is in front)
- **stores/** — Zustand stores (persisted to AsyncStorage via `zustand/middleware`). `activeWorkoutStore` (`@SparkyFitness/active-workout`) holds the currently-running workout timer state: active session snapshot, flattened `steps` list, `completedSetIds`, an `activeSetId` cursor that advances strictly forward through `steps`, and a `rest` object representing the rest period before `activeSetId` (`state: 'ready' | 'resting' | 'paused'`, absolute `endsAt`, scheduled notification id, and a monotonic `instanceToken` used to reject stale async schedule resolutions). Exposes `startWorkout`/`startWorkoutAtSet`/`jumpToSet` (forward-only)/`clearWorkout`/`completeActiveSet`/`uncompleteSet`/`pauseRest`/`resumeRest`/`dismissRest`/`markRestReady`/`reconcileWithSession`. Completing the final set leaves `activeSetId` null and `rest` in `ready` — the workout stays on screen as "Complete" until the user explicitly clears it.
...
**Workout timer (rest timer HUD)**: Live rest timer and set-completion tracker for preset workouts. State lives in `stores/activeWorkoutStore` (zustand with AsyncStorage persist) so it survives app backgrounding and cold starts. The store is organized around an `activeSetId` cursor (the set the user is currently on) and a `rest` object (`ready`/`resting`/`paused`) representing the rest period *before* `activeSetId` — completing a set advances the cursor and immediately starts the rest for the next set. `ActiveWorkoutBar` renders as a sibling of the root navigator (not inside a screen) so it floats above every screen — it uses the shared `navigationRef` from its module to deep-link into `WorkoutDetail` for the active session. `WorkoutDetailScreen` reads/writes the store to drive the set-complete and rest timer UI: tap on the active set completes and advances, tap on a completed set unchecks it (without moving the cursor), long-press on a later set confirms a forward-only jump. Rest notifications are scheduled via `services/notifications` using `expo-notifications` on both platforms plus `expo-haptics` for completion haptics; the Android `workout-timer` channel is set up in `initNotifications()`, called from `App.tsx` on mount. Set IDs on a preset workout are preserved across edits (server-side in `exerciseService`) so the active cursor and completed-set state stay correctly bound to rows after an inline edit.
